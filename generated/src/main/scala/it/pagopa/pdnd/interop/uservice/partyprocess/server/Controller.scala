...
import it.pagopa.pdnd.interop.uservice.partyprocess.api.HealthApi
import it.pagopa.pdnd.interop.uservice.partyprocess.api.PlatformApi
...
class Controller(health: HealthApi, platform: PlatformApi, process: ProcessApi, validationExceptionToRoute: Option[ValidationException => Route] = None)(implicit system: ActorSystem) {
...
path("pdnd-interop-uservice-party-process" / "build-info") {
get {
complete(HttpEntity(ContentTypes.`application/json`, buildinfo.BuildInfo.toJson))
}
}

lazy val routes: Route = getBuildInfo ~ pathPrefix("pdnd-interop-uservice-party-process" / interfaceVersion) {
toStrictEntity(strictnessTimeout) {
extractRequest {
request =>
validationFunction(request){
health.route  ~ platform.route  ~ process.route 
}
