    ...
    entity(as[Multipart.FormData]).flatMap { formData =>
      extractRequestContext.flatMap { ctx =>
        ...
        val uploadingSink: Sink[FormData.BodyPart, Future[PartsAndFiles]] =
          Sink.foldAsync[PartsAndFiles, Multipart.FormData.BodyPart](PartsAndFiles.Empty) { (acc, part) =>
            ...
            part.filename.map { fileName =>
              fileFields
                .find(_.fieldName == part.name)
                .map { case FileField(_, destFn) =>
...
object MultipartDirectives extends MultipartDirectives with FileUploadDirectives {
  val tempFileFromFileInfo: FileInfo => File = { (file: FileInfo) =>
    File.createTempFile(file.fileName, ".tmp")
  }
}
...
final case class PartsAndFiles(form: immutable.Map[String, String], files: Map[String, (FileInfo, File)]) {
  ...
  def addFile(info: FileInfo, file: File): PartsAndFiles =
    this.copy(files = files.updated(info.fieldName, (info, file)))
}
