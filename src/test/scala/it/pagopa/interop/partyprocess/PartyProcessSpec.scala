...
//        .once()
      ...
      val response =
        Http()
          .singleRequest(
            HttpRequest(uri = s"$url/onboarding/info?externalId=$externalId", method = HttpMethods.GET)
          )
          .futureValue
      ...
      val response =
        Http()
          .singleRequest(
            HttpRequest(
              uri =
                s"$url/external/institutions/$externalId/relationships?productRoles=security,api&products=Interop",
              method = HttpMethods.GET
            )
          )
          .futureValue
